...
@section('content')
    ...
    @include('master-data.department.modal')
                        ...
                        <div class="col-md-9 text-md-right">
                            <a href="javascript:void(0)" onclick="addData()"
                                ...
                                <p style="font-size: 14px; font-weight: 600; display: flex; align-items: center;text-align: center !important;">
                                    ...
                                    <span style="padding-top: 1px;">Add Department</span>
                                </p>
                            </a>
                        </div>
                    </div>
    ...
    <script>
        ...
        const addData = () => {
            ...
            dsStore = 'add';
            $('.text-danger').text('');
            $('#validationDepartment').text('');
            $('#formModules')[0].reset();
            $('#moduleModalTitle').html('Add New Department')
            $('#submitBtn').html('ADD')
            $('#addModal').modal({
                backdrop: 'static',
                keyboard: false
        ...
        const editData = (id) => {
            ...
            $.ajax({
                type: "GET",
                url: "{{ route('depart.edit') }}",
                data: {
                    id : id
                },
                success: function (response) {
                    $('#moduleModalTitle').html('Edit Department');
        ...
        $('#formModules').on('submit', function(e) {
            ...
            if (!formData.department) {
                $('#validationDepartment').text('Department is required.');
                isValid = false;
            }
            ...
            if (isValid) {
                $.blockUI();
                $.ajax({
                    url: dsStore == 'add' ? '{{ route("depart.store") }}' : '{{ route("depart.update") }}',
                    method: 'POST',
                    data: formData,
                    headers: {
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#validationDepartment').text(response.message);
                            $.unblockUI();
                        } else {
                            if (response.errors) {
                                if (response.errors.module) {
                                    $('#validationDepartment').text(response.errors.module[0]);
                                }
                            }
                        }
                        $.unblockUI();
